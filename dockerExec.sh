...
echo "Running democoin inside docker with the following arguments:
  HOST:  $HOST
  PORT:  $PORT
  LOG_LEVEL:  $LOG_LEVEL
  PEERS: $PEERS
...
docker run -t --rm --name democoin_$NAME $HOST_ARG $NAME_ARG $PORT_ARG $PEERS_ARG $LOG_LEVEL_ARG -v /$(pwd):/democoin -p $PORT:$PORT democoin